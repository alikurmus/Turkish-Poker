...
class Deck(set):
    ...
    def get_cards(self, n=1):
        if len(self.cards) < n:
            self.cards = self.cards.union(self.discards)
            self.discards = set()
        cards_to_get = []
        for i in range(n):
            a_card = random.sample(self.cards, 1)[0]
            self.cards.remove(a_card)
            cards_to_get.append(a_card)
        return cards_to_get
...
class Hand:
    ...
    def __repr__(self):
        ...
        pretty_value1 = (self.desc
                         + " ("
                         + "{:.6f}".format(self.value)
                         + ")")
        pretty_discard = "D: " + "".join(
            [str(x) for x in self.card_list
             if self.card_list.index(x) in self.discard_list])
    ...
    def to_file(self):
        # This method outputs the hand in a format that can be written in a
        # csv file. We will then use that file for analysis of hands.
    
        # We will use 1 and 0 for the output of winner to make counting
        # easier for the analysis code. And since join expects a list of
        # strings we convert the mr3 values to strings and pretty format the
        # value and final_value to 6 digits.
        rep = ",".join([self.desc_mr1, self.desc_mr2, str(self.desc_mr3),
                        "{:.6f}".format(self.value), self.final_desc_mr1,
                        self.final_desc_mr2, str(self.final_desc_mr3),
                        "{:.6f}".format(self.final_value),
                        str(1 if self.winner else 0)])
        return rep
    ...
    def evaluate(self):  # returns a list of cards to be discarded
        ...
        if len(numeral_dict) == 4:
            ...
            # (1.2340 to 8.7653)
            self.value = (pair
                          + self.card_list[2].numeral / 10
                          + self.card_list[3].numeral / 100
                          + self.card_list[4].numeral / 1000
        ...
        # Two pair or 3-of-a-kind
            ...
            if 3 in numeral_dict.values():
                ...
                # unpaired card.
                # (21.30 to 87.63)
                self.value = (big_pair * 10
                              + small_pair
                              + self.card_list[4].numeral / 10
                              + self.card_list[4].suit / 100)
        ...
        # Full house or 4-of-a-kind
            ...
            if straight and not flush:
                ...
                self.value = (90
                              + self.card_list[0].numeral / 10
                              + self.card_list[1].numeral / 100
                              + self.card_list[2].numeral / 1000
                              + self.card_list[3].numeral / 10000
                              + self.card_list[4].numeral / 100000
                              + t_card_suit / 1000000)
                ...
                if high_card_dict["Almost flush"][0]:
                    ...
                    self.discard_list = high_card_dict[
                                             "Full open ended straight"][2]
    ...
    def final_evaluate(self):
        ...
        if (len(self.final_card_list)) == 4:
            print("wtf!")
    
        # This function has the same flow and logic as the evaluate method.
        # The comments are identical but they are there to to avoid having to
        # go to the corresponding code in the evaluate method.
        # This can probably be better organized in a single function but this
        #  way keeps the code more readable in my opinion.
        key_value_sort = operator.attrgetter("numeral")
        self.final_card_list.sort(key=key_value_sort, reverse=True)
        # Clean the sort order of cards that is leftover from the initial
        # evaluation
        for card in self.final_card_list:
            card.sort_order = 0
        ...
        numeral_dict = collections.defaultdict(int)
        suit_dict = collections.defaultdict(int)
        for my_card in self.final_card_list:
            numeral_dict[my_card.numeral] += 1
            suit_dict[my_card.suit] += 1
    
        # Pair
        # If we have 4 different numeral values in the hand that means we
        # have two of a certain numeral value therefore we have a pair.
        if len(numeral_dict) == 4:
            # We are going to use two dictionaries to count the occurrences of
            # each numeral and suit in the hand. The reason we are using
            # collections.defaultdict instead of a regular dictionary is that
            # collections.defaultdict initiates each key with a default value
            # (0 for ints in our case). We could have used a regular dictionary
            # and reference it by using the get method and a default value
            # But collections.defaultdict makes the rest of the code more
            # readable by using it here just once.
            pair = list(numeral_dict.keys())[
                list(numeral_dict.values()).index(2)]
            ...
            # The hand is already sorted in descending order by numeral value.
            # What we want is to put the pair first (since it is the most
            # significant factor in the value of the hand and then put the rest
            # of the cards in descending order. So A K 9 9 7 looks like
            # 9 9 A K 7. This is important as well because we are going to use
            # the sorted values of these cards to distinguish between two hands
            # with the same pairs. Like 9 9 A K 7 and 9 9 Q J 8.
            for index, c in enumerate(self.final_card_list):
                if c.numeral == pair:
                    # 10 is large number that will make sure that the pair
                    # will be sorted before the high cards
                    # (the highest high card will have an index value of 5).
                    self.final_card_list[index].sort_order = 10
                else:
                    # We subtract the index from 5 because in initial reverse
                    # sorting by numeral the indices of bigger numerals
                    # became smaller.
                    self.final_card_list[index].sort_order = 5 - index
            ...
            # Descriptions
            self.final_desc = "A pair of " + str(L_NUMERAL_LIST[pair]) + "s"
            self.final_desc_mr1 = "Pair"
            self.final_desc_mr2 = str(D_NUMERAL_LIST[pair])
            # Numeral list starts with an Ace, we want the numeral of 3rd card.
            self.final_desc_mr3 = str(D_NUMERAL_LIST[3])
            
            # This is the numeric value
            # pair's numeral + 3 decimals + 1 decimal for the suit of the
            # largest unpaired card.
            # (1.234 to 8.765)
            self.final_value = (pair
                                + self.final_card_list[2].numeral / 10
                                + self.final_card_list[3].numeral / 100
                                + self.final_card_list[4].numeral / 1000
                                + self.final_card_list[2].suit / 10000)
        ...
        # Two pair or 3-of-a-kind
            ...
            if 3 in numeral_dict.values():
                ...
                # Descriptions
                self.final_desc = "Three " + str(L_NUMERAL_LIST[trips]) + "s"
                self.final_desc_mr1 = "Trips"
                self.final_desc_mr2 = str(D_NUMERAL_LIST[trips])
                self.final_desc_mr3 = str(D_NUMERAL_LIST[4])

                # Three of a kind's numeral * 100.
                # (100 to 800)
                self.final_value = trips * 100
            else:
                # Two pairs
                # We will first find the first pair (list.index returns the
                # first match)
                ...
                for index, c in enumerate(self.final_card_list):
                    if c.numeral == big_pair:
                        ...
                        self.final_card_list[index].sort_order = 9
                    else:
                        self.final_card_list[index].sort_order = 5 - index
                ...
                # Descriptions
                self.final_desc = "Two Pairs: " + str(
                    L_NUMERAL_LIST[big_pair]) + "s over " + str(
                    L_NUMERAL_LIST[small_pair]) + "s"
                self.final_desc_mr1 = "Two Pairs"
                self.final_desc_mr2 = str(D_NUMERAL_LIST[big_pair])
                self.final_desc_mr3 = str(D_NUMERAL_LIST[small_pair])

                # larger pair's numeral * 10 + smaller pair's numeral +
                # decimal + 1 decimal for the suit of the unpaired card.
                # (21.30 to 87.63)
                self.final_value = (big_pair * 10 + small_pair
                                    + self.final_card_list[4].numeral / 10
                                    + self.final_card_list[4].suit / 100)
        ...
        # Full house or 4-of-a-kind
            ...
            if 2 in numeral_dict.values():
                ...
                for index, c in enumerate(self.final_card_list):
                    if c.numeral == trips:
                        ...
                        # Cards that are not port of the trips are part of
                        # the pair so we can assign them the
                        # same sort_order value.
                        self.final_card_list[index].sort_order = 5
                key_value_sort = operator.attrgetter("sort_order")
                self.final_card_list.sort(key=key_value_sort, reverse=True)

                # Descriptions
                self.final_desc = "Full House " + str(
                    L_NUMERAL_LIST[trips]) + "s"
                self.final_desc_mr1 = "Full House"
                self.final_desc_mr2 = str(D_NUMERAL_LIST[trips])
                self.final_desc_mr3 = str(D_NUMERAL_LIST[pair])

                # Three of a kind's numeral * 1000 (pair's numeral is not
                # necessary).
                # (1000 to 8000)
                self.final_value = trips * 1000
                ...
                # Descriptions
                self.final_desc = "Four of a kind " + str(
                    L_NUMERAL_LIST[four]) + "s"
                self.final_desc_mr1 = "Four of a kind"
                self.final_desc_mr2 = str(D_NUMERAL_LIST[four])
                self.final_desc_mr3 = str(D_NUMERAL_LIST[5])

                # numeral value of the four of a kind * 10000.
                # (10000 to 80000)
                self.final_value = four * 10000
            ...
            # numerals with a count more than 1 before where we checked for
            # pairs etc.
            min_numeral = self.final_card_list[4].numeral
            max_numeral = self.final_card_list[0].numeral
            if int(max_numeral) - int(min_numeral) == 4:
                straight = True
            ...
            if len(set(numeral_dict.keys()).difference(low_straight)) == 0:
                ...
                # We are using the Ace as a 6, so we change its value from 8
                # to 0 to sort correctly.
                self.final_card_list[0].numeral = 0
                # We reorder the deck to take into account the new value of
                # the Ace.
                self.final_card_list.sort(key=key_value_sort, reverse=True)
            ...
            if straight and not flush:
                # The suit of the T matters because if there are two straights
                # that begin with the same numeral the suit of the T decides
                # the winner. Each straight must have a T. So find the card
                # with the numeral value 4 (which is the numeral value of T).
                for c in self.final_card_list:
                    if c.numeral == 4:
                        t_card = c
                        # Find the suit of the T
                        t_card_suit = t_card.suit
                ...
                # Descriptions
                self.final_desc_mr1 = "Straight"
                self.final_desc_mr3 = str(D_SUIT_LIST[t_card_suit])
                if not ace_low:
                    self.final_desc = "Straight " + str(
                        L_NUMERAL_LIST[max_numeral]) + " high"
                    self.final_desc_mr2 = str(D_NUMERAL_LIST[max_numeral])
                else:
                    self.final_desc = "Straight Ace Low"
                    self.final_desc_mr2 = high

                # 90 + 5 decimals + 1 decimal for the suite of the T
                # (90.543210 to 90.876543)
                self.final_value = (90
                                    + self.final_card_list[0].numeral / 10
                                    + self.final_card_list[1].numeral / 100
                                    + self.final_card_list[2].numeral / 1000
                                    + self.final_card_list[3].numeral / 10000
                                    + self.final_card_list[4].numeral / 100000
                                    + t_card_suit / 1000000)
                ...
                # the winner. Since all cards in the flush have the same suit,
                # we can look at any card in the hand.
                flush_suit = self.final_card_list[0].suit

                # Descriptions
                self.final_desc = "Flush " + str(
                    L_NUMERAL_LIST[max_numeral]) + " high"
                self.final_desc_mr1 = "Flush"
                self.final_desc_mr2 = str(D_NUMERAL_LIST[max_numeral])
                self.final_desc_mr3 = str(D_SUIT_LIST[flush_suit])

                # 9000 + 5 decimals + 1 decimal for the suite.
                # (9000.643210 to 9000.876533)
                self.final_value = (9000
                                    + self.final_card_list[0].numeral / 10
                                    + self.final_card_list[1].numeral / 100
                                    + self.final_card_list[2].numeral / 1000
                                    + self.final_card_list[3].numeral / 10000
                ...
                # The suit of the straight flush matters because if there are
                # two that start with the same numeral the suit of the straight
                # flush decides the winner. Since all cards in the straight
                # flush have the same suit, we can look at any card in the hand.
                flush_suit = self.final_card_list[0].suit

                # Descriptions
                self.final_desc_mr1 = "Straight Flush"
                self.final_desc_mr3 = str(D_SUIT_LIST[flush_suit])
                if not ace_low:
                    self.final_desc = "Straight Flush " + str(
                        L_NUMERAL_LIST[max_numeral]) + " high"
                    self.final_desc_mr2 = str(D_NUMERAL_LIST[max_numeral])
                else:
                    self.final_desc = "Straight Flush Ace Low"
                    self.final_desc_mr2 = high

                # 100000 + 5 decimals + 1 decimal for the suite.
                # (100000.643210 to 100000.876533)
                self.final_value = (100000
                                    + self.final_card_list[0].numeral / 10
                                    + self.final_card_list[1].numeral / 100
                                    + self.final_card_list[2].numeral / 1000
                                    + self.final_card_list[3].numeral / 10000
                ...
                # hands with the same numerals the suit of the highest card
                # decides the winner.
                # The highest card is in index 0 since the hand is sorted.
                high_card_suit = self.final_card_list[0].suit

                # Descriptions
                self.final_desc = "High card"
                self.final_desc_mr1 = "High card"
                self.final_desc_mr2 = str(D_NUMERAL_LIST[max_numeral])
                self.final_desc_mr3 = str(
                    D_NUMERAL_LIST[self.final_card_list[1].numeral])

                # 0 + 5 decimals + 1 decimal for the suite of the highest card.
                # (0.12346 to 0.87653)
                self.final_value = (self.final_card_list[0].numeral / 10
                                    + self.final_card_list[1].numeral / 100
                                    + self.final_card_list[2].numeral / 1000
                                    + self.final_card_list[3].numeral / 10000
                                    + self.final_card_list[4].numeral / 100000
    ...
    def discard(self):
        for i, card in enumerate(self.card_list):
            # Add cards that are not in the discard list to the final form of
            # the hand
            if i not in self.discard_list:
                self.final_card_list.append(card)

    def draw(self, deck):
        # Initially, each player can only draw 3 cards at most. This is to avoid
        # situations where earlier drawing players draw so many cards that
        # the last player has no cards to draw from. If after this round of 3
        #  cards each there are players who need more cards to make their
        # hands up to 5 cards, they are given these cards in the
        # draw_from_discards method.
        if len(self.discard_list) > 0:
            self.final_card_list.extend(
                deck.get_cards(min(3, len(self.discard_list))))
        return deck

    def draw_from_discards(self, deck):
        # This method tops up all the players hands to 5 cards. If there were
        #  initially more than 12 discards, there will be no cards left. In
        # real life this would be handled by reshuffling the discarded cards
        # and giving to the players who are short of cards. In this class,
        # we just add the discards to the deck of cards when we run out of
        # cards--which will give slightly different results
        while len(self.final_card_list) < 5:
            self.final_card_list.extend(deck.get_cards(1))
        return deck
...
# Loop for number of games, received from the command line arguments
counter = 0
while counter < games:
    ...
    for player in range(4):
        cards.append(game_deck.get_cards(5))
        hands.append(Hand(cards[player]))
    
    # Evaluate hands.
    discards = []
    someone_opens = False
    for hand in hands:
        # Evaluate hands and peek and build the list of total discards.
        discards.extend(hand.evaluate())
        # Check if anyone can open the game.
        if hand.value >= OPENER_LIMIT:
            someone_opens = True
    game_deck.discards = discards
    
    # See if anyone can open, if not, we don't count this game.
    if someone_opens:
        counter += 1
        for hand in hands:
            # Discard cards in the discard list making the final hand with
            # only cards that are not in the discard list.
            hand.discard()
            # Draw cards from the deck and modify the deck to reflect the
            # drawn cards.
            game_deck = hand.draw(game_deck)
    
        # Do the secondary draw to top up players with short hands. See
        # Hand.draw and Hand.draw_from_discards comments for details.
        for hand in hands:
            game_deck = hand.draw_from_discards(game_deck)
    
        # All hands are in their final form. Evaluate each and declare the
        # winner.
        scores = []
        for hand in hands:
            scores.append(hand.final_evaluate())
        for hand in hands:
            if hand.final_value == max(scores):
                hand.winner = True
    
        # Output to screen or file.
        if to_file:
            for hand in hands:
                out_file.write(str(counter) + "," + hand.to_file() + "\n")
        else:
            print("-" * 80)
